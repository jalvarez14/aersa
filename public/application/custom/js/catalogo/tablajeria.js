(function ($) {
    ...
    $.tablajeria = function (container, options) {
        ...
        plugin.edit = function (count) {
            ...
            $('#producto_add').on('click', function () {
                var tr = $('<tr>');
                tr.append('<td><input type="hidden"  name=plantillatablajeria_detalle[' + count + '][idproducto] value="' + $('input#idproducto').val() + '">' + $('input#producto_autocomplete').typeahead('val') + '</td>');
                tr.append('<td><a href="javascript:;"><i class="fa fa-trash"></i></a></td>');
                //INSERTAMOS EN LA TABLA
                $('#productos_table tbody').append(tr);
                //LIMPIAMOS EL AUTOCOMPLETE
